...
function dac!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(EP, eDAC_heat_consumption[y in DAC_ID, t = 1:T], vCO2_DAC[y,t] * dfDac[y,:Heat_GJ_per_CO2_metric_ton]) ## MMBTU / hour = tCO2 removed / hour * MMBTY / tCO2
    ...
    for z in 1:Z
        ...
        ## constraint that energy used by all DAC facilities must EQUAL total energy diverted from nuclear plants 
        @constraints(EP, begin
            [t in 1:T], sum(EP[:eDAC_heat_consumption][y, t] for y in nuke_dac) == sum(vsteam_heat_DAC[t,zz] for zz in z)*MW_to_GJ_conversion
        end) ## total MMBTU / hour for all plants == total MW diverted * GJ / MW conversion

        ## diverted energy from nuclear can't exceed capacity of heat exchangers 
        @constraints(EP, begin
        ...
        end) ## total MW diverted for each hour <= total max heat exchanger capacity per nuclear DAC plant 

        ## can't have more capacity for nuclear gens injecting into grid and supplying DAC than the installed capacity
        @constraints(EP, begin
        ...
        end) ## total energy injected into the grid by all nuclear generators and energy diverted from those gens for DAC must be <= the total installed capacity of all these gens
    ...
    ## max heat exchanger capacity for all DAC must be <= total diverted steam * unit heat / unit steam (heat)

    ### heatpump constraints? 
    ...
    ## tCO2 emitted from heat for DAC = MMBTU/hr  * tCO2 / heat resource (fuel type) * (1 - % tCO2 captured / heat resource)
    ## THE POSITIVE EMISSIONS FOR HEAT FOR DAC (later the negative emissions are subtracted to find the net emissions) 
    ...
    ## DAC electricity demand increases system demand!!!!!
    
    ## total power demand of DAC must be less than or equal to RE generation FOR EACH HOUR
    ## this is on the right track, but need to use vP instead of dfGen, 
    ## need to constrain by zone, and need to make sure it's additional instead of existing

    ## vP in EP: identify indexes of RE in vP (power generation per hour) and match the zones 

    ## heatpump: (dac heat demand / coeff of performance = 2) = electricity in + DAC power demand <= power output of electricity source
    ## how do we identify RE gens (indexes)?
        ## RE generation: VRE, geothermal, hydro --> add storage as well? (technically RE, but could be storing energy from fossil, so could still increase emissions)
        ## vre_gens = intersect(inputs["VRE"], dfGen[dfGen[!,:Zone].==z,:R_ID]) # all VRE gens (R_IDs) in the current zone z
        ## geothermal_gens = intersect(inputs["geothermal"], dfGen[dfGen[!,:Zone].==z, :R_ID]) # all geothermal gens in the current zone z 
        ## all_re_gens = [vre_gens; geothermal_gens]
    ## how do we define new and additional RE? differentiate existing RE vs new RE for DAC
    ## deliverability defined by zone, need to constrain by zone 

    ## increase capacity to 10Mt
    ...
    ## FIXED COSTS included in the 800 in dac.csv -- forget about these!!!
