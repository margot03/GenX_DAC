...
function write_dac_capacity(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	# Eligible gens
	# el_gens = inputs["eligible_gens"]
	# CSV.write(joinpath(path, "dac_eligible_gens.csv", el_gens))
	...
	cFix = (!isempty(inputs["dfDac"]) ? value(EP[:eTotalCFixedDAC])*scale_factor : 0.0)
	cFix_E = (!isempty(inputs["dfDac"]) ? value(EP[:eTotalCFixedDAC_Energy])*scale_factor^3 : 0.0)
	...
	## NEED TO ADD ELECTRICITY COST TO TOTAL COST HERE (and maybe to variable cost above)
	...
	## dac results to rescale with scale_factor
	# eCFixed_DAC_Energy
